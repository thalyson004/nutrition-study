...
def calcular_estatisticas_comportamento(dietas):
    ...
    for plano in dietas:
        for dia, refeicoes in plano.items():
            if not isinstance(refeicoes, dict): continue
            total_dias += 1
            assinatura_dia = []
            ...
            for nome_refeicao, itens in refeicoes.items():
                ...
                alimentos = [str(item.get('alimento', '')).strip().lower() for item in itens if item.get('alimento')]
...
# ==========================================
def processar_arquivos(arquivos_dietas, arq_mapa_tbca, arq_tbca_completo, arq_mapa_pegadas):
    ...
    for arquivo_dieta in arquivos_dietas:
        ...
        qtd_dias_unicos, total_dias, taxa_arroz_feijao = calcular_estatisticas_comportamento(dietas)
                ...
                for nome_refeicao, itens in refeicoes.items():
                    ...
                    for item in itens:
                        ...
                        if cod and cod in tbca_completo:
                            ...
                            if alimento: itens_sem_tbca.add(alimento)

                        # 2. Pegadas Ambientais
                        if alimento in mapa_pegadas:
                            fator_p = qtd / FATOR_DIVISAO_PEGADA
                            peg = mapa_pegadas[alimento]
                            nutrientes_ref["Pegada de Carbono"] += float(peg.get('carbon_footprint', 0)) * fator_p
                            nutrientes_ref["Pegada Hídrica"] += float(peg.get('water_footprint', 0)) * fator_p
                            nutrientes_ref["Pegada Ecológica"] += float(peg.get('ecological_footprint', 0)) * fator_p
                        else:
                            if alimento: itens_sem_pegada.add(alimento)
        ...
        with open(arquivo_saida_txt, "w", encoding="utf-8") as f_out:
            ...
            def formatar_saida(titulo, dados_dict):
                ...
                nutrientes_foco = [
                    "Energia", "Pegada de Carbono", "Pegada Hídrica", "Pegada Ecológica", 
                    "Carboidrato disponível", "Carboidrato total", "Proteína", 
                    "Lipídios", "Fibra alimentar", "Sódio"
                ]
                
                # Garante a ordem correta das refeições
                ordem_ref = ["Geral", "Café da Manhã", "Lanche da Manhã", "Almoço", "Lanche da Tarde", "Jantar", "Ceia"]
                categorias = [o for o in ordem_ref if o in dados_dict]
                for c in dados_dict.keys():
                    if c not in categorias: categorias.append(c)
                ...
                for categoria in categorias:
                    ...
                    for nutri in chaves:
                        ...
                        if media > 0.1: # Filtro original do seu script
                            un = "g"
                            if nutri == "Energia": un = "kcal"
                            elif nutri == "Sódio": un = "mg"
                            elif "Pegada" in nutri: un = ""  # Pegadas ficam com a unidade limpa no visual
            ...
            if itens_sem_tbca or itens_sem_pegada:
                ...
                if itens_sem_tbca:
                    ...
                    for i in sorted(itens_sem_tbca): log(i)
                if itens_sem_pegada:
                    log("\n--- Não encontrados nas Pegadas ---")
                    for i in sorted(itens_sem_pegada): log(i)
...
arquivos_para_analisar = [
    "corrigido-dietas-regular.json",
    "corrigido-dietas-vegana.json",
    "corrigido-dietas-vegetariana.json"
]
