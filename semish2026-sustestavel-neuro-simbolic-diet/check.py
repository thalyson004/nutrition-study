...
def verificar_quantidade_dietas(caminho_arquivo):
    ...
    try:
        ...
        if isinstance(dietas, list):
            ...
            print(
                f"Sucesso! O arquivo '{caminho_arquivo}' contém {quantidade} dietas válidas."
            )
            return quantidade
        else:
            print(
                "Aviso: O JSON foi carregado, mas a estrutura principal não é uma lista (array)."
            )
        ...
        print(
            "Dica: Verifique se há vírgulas separando cada dieta e se o arquivo começa com '[' e termina com ']'."
        )
        return 0
...
for dieta in ["regular", "vegetariana", "vegana"]:
    nome_do_arquivo = f"dietas-{dieta}.json"
